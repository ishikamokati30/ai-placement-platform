...
const normalizeCustomFields = (value) => {
  ...
  return value
    .map((field) => ({
      label: normalizeText(field?.label, "").slice(0, 80),
      value: normalizeText(field?.value, "").slice(0, 2000),
    }))
...
const startInterview = async (req, res) => {
  ...
  const company = normalizeText(body.companyName || body.company, "");
  const round = normalizeText(body.currentRound || body.round, "");
  const resumeText = body.resumeText || "";
  const customFields = normalizeCustomFields(body.customFields);
  const preferredTopics = normalizeStringList(body.preferredTopics);
  const storedTopic =
  ...
  try {
    ...
    const previousQuestions = await interviewService.getRecentQuestions(
      interview.id
    );
...
const submitAnswer = async (req, res) => {
  try {
    ...
    try {
      feedback = normalizeFeedback(
        await aiService.evaluateAnswer(question, answer, {
          ...
          preferredTopics: normalizeStringList(preferredTopics),
    ...
    const topic = interview.topic || companyName || company || "general computer science";
...
const getSetupSuggestions = async (req, res) => {
  try {
    const body = req.body && typeof req.body === "object" ? req.body : {};
    const suggestions = await aiService.generateSetupSuggestions({
      company: normalizeText(body.companyName || body.company, ""),
      role: normalizeText(body.role, ""),
      interviewType: normalizeText(body.interviewType || body.type, "technical"),
      customFields: normalizeCustomFields(body.customFields),
      preferredTopics: normalizeStringList(body.preferredTopics),
    });

    res.json(suggestions);
  } catch (error) {
    console.error("[Interview] getSetupSuggestions failed", {
      message: error.message,
    });
    res.status(502).json({
      message: "Unable to generate interview setup suggestions.",
    });
  }
};
